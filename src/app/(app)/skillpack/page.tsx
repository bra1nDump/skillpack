...
function CodeBlock({ children }: { children: React.ReactNode }) {
  return (
    <div className="mt-5 overflow-x-auto rounded border border-[var(--dark-border)] bg-[var(--dark-bg)] p-4 font-mono text-[11px] leading-[1.7] text-gray-400 sm:text-[12px]">
      {children}
    </div>
  );
}
...
export default function SkillPackPage() {
  return (
    <>
      <DarkPageHeader
        title="SkillPack"
        subtitle="One command analyzes your codebase — framework, dependencies, structure — then selects the highest-rated skills from the catalog, resolves conflicts, and installs everything."
      >
        {/* Quick install */}
        <div className="mt-5 flex w-full items-center gap-2 overflow-x-auto rounded-lg border border-[#262626] bg-[#0A0A0A] px-4 py-3 sm:inline-flex sm:w-auto sm:gap-3 sm:px-5">
          <span className="font-mono text-[12px] text-[#737373] sm:text-[14px]">$</span>
      ...
      <main className="mx-auto w-full max-w-4xl px-6 py-10 sm:px-8">
        ...
        <Section id="how-it-works" label="How It Works" title="From zero to fully equipped in one command">
          ...
          <CodeBlock>
            ...
            <br />
            <span className="text-gray-500">  Next.js 15 + Prisma + Vitest</span>
            <br />
            <br />
            <span className="text-white">● Recommended for your stack:</span>
            <br />
            <br />
            <span className="text-gray-500">  Coding CLI</span>
            <br />
            <span className="text-white">  Claude Code</span>
            <span className="ml-2 text-gray-600">Trust: 87 · ★ 40K+</span>
            <br />
            <span className="text-white">  Playwright MCP</span>
            <span className="ml-2 text-gray-600">Trust: 74 · ★ 8K+</span>
            <br />
            <br />
            <span className="text-gray-500">  UX & Design</span>
            <br />
            <span className="text-white">  Framelink</span>
            <span className="ml-2 text-gray-600">Trust: 71 · ★ 2K+</span>
            <br />
            <br />
            <span className="text-gray-500">  Documentation</span>
            <br />
            <span className="text-white">  Context7</span>
            <span className="ml-2 text-gray-600">Trust: 68 · ★ 5K+</span>
            <br />
            <br />
            <span className="text-emerald-500">4 skills matched — ranked by evidence</span>
          </CodeBlock>
        </Section>
        ...
        <Section id="analysis" label="Analysis" title="What SkillPack reads from your project">
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {[
              { file: "package.json", desc: "Framework, dependencies, scripts, dev tools" },
              { file: "requirements.txt / pyproject.toml", desc: "Python stack, ML libraries, CLI tools" },
              { file: "tsconfig / next.config / vite.config", desc: "Build system, framework version, plugins" },
              { file: "Directory structure", desc: "App router vs pages, monorepo layout, test directories" },
              { file: ".env / docker-compose", desc: "Infrastructure, databases, external services" },
              { file: "Existing skills", desc: "Already installed skills to avoid duplicates" },
            ].map((item) => (
              <div
                key={item.file}
        ...
        <Section id="quality" label="Quality" title="Only skills that earn their place">
          ...
          <ul className="mt-5 space-y-3">
            {[
              { check: "Trust Score ≥ 50", desc: "Composite of freshness, community growth, adoption, and evidence quality" },
              { check: "Above the cut line", desc: "Editorially reviewed and recommended in its category" },
              { check: "Active maintenance", desc: "Repository pushed within the last 90 days" },
              { check: "Real evidence", desc: "Independent artifacts — not just self-reported claims" },
            ].map((item) => (
              <li
                key={item.check}
        ...
        {/* Bottom CTA */}
        <div className="mt-10 flex flex-col gap-4 rounded-lg bg-[#171717] p-5 text-white sm:flex-row sm:items-center sm:justify-between sm:p-6">
          <div>
          ...
          <div className="shrink-0 overflow-x-auto rounded border border-[#262626] bg-[#0A0A0A] px-4 py-2.5 font-mono text-[12px] sm:px-5 sm:text-[13px]">
